...
test.describe('ADO-36567 Domain Table Status', () => {
  ...
  test('verify domain status chips color', async ({ assetPage }) => {
    ...
    const forecastStatusSuccessColor = await assetPage.getDomainChipColor('On Track');
    const currentStatusErrorColor = await assetPage.getDomainChipColor('Threshold');
    const forecastStatusErrorColor = await assetPage.getDomainChipColor('Off Track');
    const currentStatusWarningColor = await assetPage.getDomainChipColor('Trigger');
    const forecastStatusWarningColor = await assetPage.getDomainChipColor('At Risk');
    ...
    expect(forecastStatusSuccessColor, `Domain forecast status success  color is green ${forecastStatusSuccessColor}`).toBe('rgb(0, 116, 97)');
    expect(currentStatusErrorColor, `Domain current status error color is red ${currentStatusErrorColor}`).toBe('rgb(202, 35, 32)');
    expect(forecastStatusErrorColor, `Domain forecast status error color is red ${forecastStatusErrorColor}`).toBe('rgb(202, 35, 32)');
    ...
    expect(forecastStatusWarningColor, `Domain forecast status warning color is orange ${forecastStatusWarningColor}`).toBe('rgb(191, 77, 0)');
  });
  test('verify sub domain status chips color', async ({ assetPage, fxDomains }) => {
    ...
    const forecastStatusSuccessColor = await assetPage.getSubdomainChipColor('On Track');
    const currentStatusErrorColor = await assetPage.getSubdomainChipColor('Threshold');
    const forecastStatusErrorColor = await assetPage.getSubdomainChipColor('Off Track');
    const currentStatusWarningColor = await assetPage.getSubdomainChipColor('Trigger');
    const forecastStatusWarningColor = await assetPage.getSubdomainChipColor('At Risk');
    ...
    expect(forecastStatusSuccessColor, `Sub-domain forecast status success color is light green ${forecastStatusSuccessColor}`).toBe('rgb(230, 241, 239)');
    ...
    expect(forecastStatusErrorColor, `Sub-domain forecast status error color is light red ${forecastStatusErrorColor}`).toBe('rgb(250, 233, 233)');
    ...
    expect(forecastStatusWarningColor, `Sub-domain forecast status warning color is light orange ${forecastStatusWarningColor}`).toBe('rgb(255, 248, 225)');
  });
});
